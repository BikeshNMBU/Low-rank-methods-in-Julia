...
mutable struct RADynamicLowRankLayer{T<:AbstractFloat}
    ...
    function RADynamicLowRankLayer(input_size::Int, output_size::Int, rank::Int, tol::T, activation::Function; dtype::Type{T}=Float32) where T
        ...
        new{T}(U, S, V, U1, V1, b, rMax, rCurrent, tol, activation, U_m, S_m, V_m, b_m)
    end
...
function sgd_update_with_momentum_and_clipping!(layer::RADynamicLowRankLayer, grads, lr, momentum, threshold, dlrt_step="basis")
    
    r = layer.rCurrent
    if dlrt_step == "basis"
        # Compute gradients and apply gradient clipping
        dU, dV, db = grads[layer.U], grads[layer.V], grads[layer.bias]  
        ...
        dK = layer.U_momentum[:, 1:r] * S0
	    K .-= lr * dK
        K_ext = hcat(K, U0)
        #K_ext = hcat(U0, dK) # Extended K for QR decomposition
        ...
        dL = layer.V_momentum[:, 1:r] * S0'
	    L .-= lr * dL
        L_ext = hcat(L, V0)
        #L_ext = hcat(V0, dL) # Extended L for QR decomposition
        ...
        # Update bias
        layer.bias .-= lr * layer.b_momentum
    ...
    elseif dlrt_step == "coefficients"
        ...
        layer.S[1:layer.rCurrent, 1:layer.rCurrent] .-= lr * layer.S_momentum[1:layer.rCurrent, 1:layer.rCurrent]
